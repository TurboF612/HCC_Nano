...
gc()
{
  ...
  green <- "#66C2A5"
  gray <- "#D9D9D9"
  cor_set <- c("#C994C7", "#66C2A5", "#AEDDEE", "#80B1D3", "#3477A9", 
...
library(ggplot2)
library(tidyverse)
fig1a_data <- read_tsv("fig1a.txt")

cell_centers <- aggregate(cbind(UMAP_1, UMAP_2) ~ CellType, data = fig1a_data, FUN = mean)

ggplot(fig1a_data, aes(x = UMAP_1, y = UMAP_2, color = CellType)) +
  geom_point(size = 0.1) +
  scale_color_manual(values = cor_set) +
  geom_text(data = cell_centers, aes(x = UMAP_1, y = UMAP_2, label = CellType), 
...
ggsave("fig1a.pdf", height = 8, width = 8)
...
PieDonutCustom <- function (data, mapping, start = getOption("PieDonut.start", 
                            ...
                            family = getOption("PieDonut.family", ""), palette_name="Set3")
{
  (cols = colnames(data))
  if (use.labels) 
...
fig1b_data <- read_tsv("fig1b.txt")
PieDonutCustom(fig1b_data,
               aes(site, CellType, count = n),
               r0=0.1,r1=1,r2=1.3,explodeDonut=TRUE,ratioByGroup=FALSE,
               title="Cell Proportions",
               titlesize = 7,
               labelposition = 2,
               pieLabelSize = 3.5,
               donutLabelSize = 2,
               pieAlpha = 0.8, donutAlpha = 0.75, 
               start = 4*pi/2,labelpositionThreshold=0.005,maxx=1.5,
               palette_name = "Set1")

#### fig1c-d ####
load("fig1c.Rda")
plotNhoodGraphDA(milo, milo.res.1, alpha = 0.05)
ggsave("fig1c.pdf", width = 8, height = 8)

load("fig1d.Rda")
p1 <- plotDAbeeswarm(milo.res.1, group.by = "celltype")
p1
ggsave("fig1d.pdf", width = 8, height = 8)

#### fig1e ####
fig1e <- read_tsv("fig1e_left.txt")

highlight_colors <- c("Epithelials" = cor_set[1], "none" = gray)
ggplot(fig1e, aes(x = UMAP_1, y = UMAP_2, color = Highlight)) +
  geom_point(size = 0.1) +
  ...
  theme(legend.position = "none",
        ...
        axis.ticks.x = element_blank()) +
  labs(x = "UMAP_1", y = "UMAP_2")
ggsave("fig1e_left.pdf", height = 8, width = 8)

fig1e_right <- read_tsv("fig1e_right.txt")
Cell_Type <- table(fig1e_right$CellType_final)

library(lessR)
PieChart(Cell_Type, 
         data = celltype,
         hole = 0,
         fill = c("#E7E1EF", "#C994C7"),
         values_color = "black",
         main = "")

#### fig1f ####
fig1f1 <- read_tsv("fig1f1.tsv")
fig1f2 <- read_tsv("fig1f2.txt")
fig1f3 <- read_tsv("fig1f3.txt")

library(VennDiagram)
venn.plot <- venn.diagram(
  x = list("Epi Diff" = fig1f1$ID, 
           "TCGA-LIHC Diff" = fig1f2$gene_id,
           "ICGC-LIRI-JP Diff" = fig1f3$gene_id),
  category.names = c("Epi Diff", "TCGA-LIHC Diff", "ICGC-LIRI-JP Diff"),
  filename = NULL, # 不保存为文件，稍后保存为PDF
  output = TRUE,
  imagetype = "pdf",
  height = 700,
  width = 700,
...
pdf("fig1f1.pdf", width = 10, height = 10)
grid.draw(venn.plot)
dev.off()
